...
class BoundaryMetrics:
    ...
    def round(self, digits: int = 2) -> "BoundaryMetrics":
        return replace(
            self, **{f.name: round(getattr(self, f.name), digits) for f in fields(self)}
        )
...
def evaluate_boundary_metrics(
    ...
    for sp, tp in tqdm(
        ...
        boundaries = [
            round(b / WAVLM_FRAME_RATE + constant_shift, 2) for b in boundaries
        ]

        seg, ref = split_utterance(
            boundaries,
            textgrid.get_tier_by_name(tier),
            tolerance=tolerance,
        )
        segs.extend(seg)
        refs.extend(ref)
    ...
    r_value = get_r_value(precision, recall)
...
def get_r_value(precision: float, recall: float) -> float:
