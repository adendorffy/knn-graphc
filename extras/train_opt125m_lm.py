...
class TrainConfig:
    ...
    # --- Checkpointing ---
    checkpoint_every_n_global_steps: int = 1000
...
class TokenizedUnitsUtteranceDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        segments = torch.from_numpy(np.load(segments_path)).long()
        units = segments[:, 2]
        tokens = self.tokenize_fn(units)
        return tokens
...
class Trainer:
        ...
        while True:
            for train_loss in self.train_epoch():
                ...
                if (
                    ...
                    % self.train_cfg.checkpoint_every_n_global_steps
                    == 0
                ):
                    self.save_checkpoint(f"step-{self.current_global_step}.pt")
    ...
    def save_checkpoint(self, name: str):
        checkpoint_path = Path(self.run.dir) / name
        checkpoint = {
            "model": self.model.state_dict(),
            "cfg": self.model.config.__dict__,
...
if __name__ == "__main__":
    ...
    model_cfg = OPTConfig(
        **{
            ...
            # Back to normal training regularisation
            "dropout": 0.1,
            "attention_dropout": 0.0,
    ...
    #     entity="slabbert-danel-stellenbosch-university",
    #     project="zerosyl-ulm-opt-125m",
    #     name="OPT-small-512x6-LibriSpeech960-graph9693",
    #     device="cuda",
    #     dtype=torch.float16,
    ...
    #     batch_size=4,
    #     num_workers=7,
    #     train_mmap_path="output/ulm/LibriSpeech-train-960-graph_knn100-tau0.45-gamma0.0500-k9693.bin",
    #     valid_segments_dir=(
    #         "output/inferred_segments/zerosyl/LibriSpeech/dev-clean/"
    ...
    #     ),
    #     valid_segments_pattern="**/*.npy",
    #     train_ctx_win_size=1024,
    #     lr_init=0.0,
    #     lr_max=2e-4,
    #     lr_final=2e-5,
    ...
    #     weight_decay=0.01,
    #     eps=1e-8,
    #     checkpoint_every_n_global_steps=1000,
    # )

    train_cfg = TrainConfig(
        entity="slabbert-danel-stellenbosch-university",
        project="zerosyl-ulm-opt-125m",
        name="OPT-small-512x6-LibriLight600-graph9693",

        device="cuda",
        dtype=torch.float16,

        # Same effective batch size as LibriSpeech-960
        accumulation_steps=16,
        grad_clip_max_norm=1.0,
        batch_size=4,
        num_workers=7,

        # Replace with your actual LibriLight output filename
        train_mmap_path=(
            "output/ulm/LibriLight-small-graph9k.bin"
        ),

        # Keep the same validation set and tokenizer
        valid_segments_dir=(
            "output/inferred_segments/zerosyl/LibriSpeech/dev-clean/"
            "knn_1_hnswm_32_search_64/"
            "zerosyl/inference-test/LibriSpeech/train-clean-100/"
        ...
        # Shorter schedule for the smaller corpus
        lr_init=0.0,
        lr_max=2e-4,
        lr_final=2e-5,
        n_linear_steps=500,
        ...
        checkpoint_every_n_global_steps=500,
    )

    trainer = Trainer(model_cfg, train_cfg)

    trainer.train(
        max_global_step=5_000,
